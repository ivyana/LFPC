...
# Step 2 (change_last_elements until there are no more long rules)
def step2(rules):
...
# Step 3 (just add the S0 symbol and link with S)
def step3(rules):
    if '/' in rules['S']:  
...
# Step 4 (remove_empty_productions);
def step4(rules):
...
# Step 5 (apply_chain_rule as long as chain rules are present)
